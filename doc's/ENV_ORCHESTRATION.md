...
O presente relatório detalha, em um patamar de execução, a metodologia para a gestão e o desdobramento dos containers do empreendimento em um ciclo de quatro estágios distintos: Ambiente de Desenvolvimento, Ambiente de Validação (ou Testes), Ambiente de Preparação para o Lançamento e Ambiente de Operação Final.

Adicionalmente, o texto abrange a configuração das esteiras de Integração e Entrega Contínua (CI/CD), utilizando a ferramenta GitHub Actions, que são apropriadas para cada um desses ambientes. São fornecidas as sequências de comandos, a nomenclatura das chaves de segurança (secrets) e orientações essenciais sobre boas práticas de proteção e os procedimentos de reversão (rollback) em caso de falha.
