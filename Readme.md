...
# Projeto: School Scheduler — trabalho DevopsTools

Este acervo contém o projeto que desenvolvi para a disciplina: um dispositivo de agendamento escolar escrito em Python. O propósito foi implementar um protótipo funcional que possibilite cadastrar docentes, grupos de alunos e matérias, e gerar um cronograma otimizado usando otimização linear (PuLP). A interface principal usa Tkinter e as informações são preservadas em MySQL.

Este registro resume a execução, explica como operar localmente (incluindo a GUI no Windows), descreve a rotina de CI/CD que configurei com GitHub Actions, e apresenta notas sobre operação com Docker Compose e deploy em Kubernetes.
Sumário rápido
...
Observação: `school_schedule.py` abre uma interface Tkinter se executado diretamente. Para executar apenas funções de backend, garanta que o arquivo permita execução sem abrir a GUI (veja a seção "Tornar import-safe").
...
A seguir descrevo, no meu próprio formato de trabalho académico, o que cada workflow faz e e como foi projetado.
